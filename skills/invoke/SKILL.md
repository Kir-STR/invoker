...
1. **Покажи сгруппированный чеклист модулей** (4 активных группы; «Масштабирование контекста»/runbooks — вне MVP, не предлагать). Рядом с каждым модулем — `note` из реестра `${CLAUDE_PLUGIN_ROOT}/modules.json` (реестр лежит в корне плагина, не под `skills/invoke/`).
   ...
   **Как спросить про набор — один вопрос, single-select (варианты взаимоисключающие):** `только recommended` / `+ git и PR` / `всё (11 модулей)` + свободный ввод для точечного набора (убрать/добавить любой модуль из чеклиста выше). **Не** делать пересекающиеся multiSelect-опции (`только recommended` И `+ X` одновременно) и **не** предотмечать «всё» — дефолт = recommended.

2. **Спроси, какие файлы-инструкции создать — multiSelect только по опциональным движкам:**
   - `AGENTS.md` — для инструментов, читающих AGENTS.md (напр. Codex);
   - `GEMINI.md` — для Gemini CLI.

   `CLAUDE.md` создаётся всегда — **не выводить его вариантом вовсе** (даже как «только CLAUDE.md»).
...
5. **Отчёт:** перечисли созданные файлы (из вывода helper'а). `invoke` копирует дословно и значения не заполняет. Если выбраны каркасы `safety` / `architectural-invariants` — напомни, что это скелеты под заполнение (`TODO`-разделы по подсказкам в файле), и предложи помочь собрать черновик из кода/спеки; safety-содержание оператор утверждает сам (гейт). Если в каком-то правиле остался проектно-уникальный `{{...}}` — укажи его оператору.
