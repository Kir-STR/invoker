...
Подробности создания (`git worktree add` + `pull --ff-only`), direct-to-main commit ordering, cwd-дисциплина Bash-инструмента, артефакты фичи в worktree, merge через `gh`, Windows-teardown, subagent dispatch внутри worktree — в runbook проекта или вашем planning/worktree skill (или эквиваленте).
