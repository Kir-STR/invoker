...
- Плейсхолдеры `{{...}}` оставляем **только** там, где значение проектно-уникально и нет разумного дефолта. Описание проекта (имя/стек/конвенции) — не дело invoke, его в шаблонах нет. `globs` дефолтит на `**/*`, ветка — на `main`. `{{BRANCH}}` в `.invoker/retro-template.md` — runtime-плейсхолдер будущего `retro`, не для оператора.
